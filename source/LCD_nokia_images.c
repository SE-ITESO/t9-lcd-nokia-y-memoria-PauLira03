

#include "LCD_nokia_images.h"

const uint8_t ITESO[] = {
		,
		};
uint8_t img_1[504];
uint8_t img_2[504];
uint8_t img_3[504];
uint8_t img_4[504];
uint8_t img_5[504];

AT_QUICKACCESS_SECTION_DATA_ALIGN(edma_tcd_t tcdMemoryPoolPtr[TCD_QUEUE_SIZE + 1], sizeof(edma_tcd_t));

void LCD_DMAsend(DMA_Type *base, dma_channel_t channel, _img_addr_t img)
{
	edma_transfer_config_t transferConfig;
	switch(img)
	{
		case kLCD_IMG_BASE_ADDR_1:
		    EDMA_PrepareTransferConfig(&transferConfig,
		    		   &img_1[0],
					   DATA_WIDTH,
					   SRC_OFFSET, /* source offset */
					   (void *) DAC_REG_ADDR,
					   DATA_WIDTH,
					   DEST_OFFSET,               /* dest offset */
					   MINOR_LOOP_BYTES,             /* minor loop bytes: 8*/
					   MAJOR_LOOP_COUNTS); /* major loop counts : 4 */
		break;
		case kLCD_IMG_BASE_ADDR_2:
		    EDMA_PrepareTransferConfig(&transferConfig,
		    		   &img_2[0],
					   DATA_WIDTH,
					   SRC_OFFSET, /* source offset */
					   (void *) DAC_REG_ADDR,
					   DATA_WIDTH,
					   DEST_OFFSET,               /* dest offset */
					   MINOR_LOOP_BYTES,             /* minor loop bytes: 8*/
					   MAJOR_LOOP_COUNTS); /* major loop counts : 4 */
		break;
		case kLCD_IMG_BASE_ADDR_3:
		    EDMA_PrepareTransferConfig(&transferConfig,
		    		   &img_3[0],
					   DATA_WIDTH,
					   SRC_OFFSET, /* source offset */
					   (void *) DAC_REG_ADDR,
					   DATA_WIDTH,
					   DEST_OFFSET,               /* dest offset */
					   MINOR_LOOP_BYTES,             /* minor loop bytes: 8*/
					   MAJOR_LOOP_COUNTS); /* major loop counts : 4 */
		break;
		case kLCD_IMG_BASE_ADDR_4:
		    EDMA_PrepareTransferConfig(&transferConfig,
		    		   &img_4[0],
					   DATA_WIDTH,
					   SRC_OFFSET, /* source offset */
					   (void *) DAC_REG_ADDR,
					   DATA_WIDTH,
					   DEST_OFFSET,               /* dest offset */
					   MINOR_LOOP_BYTES,             /* minor loop bytes: 8*/
					   MAJOR_LOOP_COUNTS); /* major loop counts : 4 */
		break;
		case kLCD_IMG_BASE_ADDR_5:
		    EDMA_PrepareTransferConfig(&transferConfig,
		    		   &img_5[0],
					   DATA_WIDTH,
					   SRC_OFFSET, /* source offset */
					   (void *) DAC_REG_ADDR,
					   DATA_WIDTH,
					   DEST_OFFSET,               /* dest offset */
					   MINOR_LOOP_BYTES,             /* minor loop bytes: 8*/
					   MAJOR_LOOP_COUNTS); /* major loop counts : 4 */
		break;
	}

	EDMA_TcdSetMajorOffsetConfig(
		    		   tcdMemoryPoolPtr, //param tcd A point to the TCD structure.
		    		   -MAJOR_LOOP_COUNTS, //* param sourceOffset source address offset.
					   0);//destOffset destination address offset.
	EDMA_TcdSetTransferConfig(tcdMemoryPoolPtr, &transferConfig, NULL);

	EDMA_TcdEnableInterrupts(&tcdMemoryPoolPtr[0], kEDMA_MajorInterruptEnable);
	EDMA_TcdEnableAutoStopRequest(&tcdMemoryPoolPtr[0], false);
	EDMA_InstallTCD(base, channel, &tcdMemoryPoolPtr[0]);
	EDMA_EnableChannelRequest(base, channel);
}

